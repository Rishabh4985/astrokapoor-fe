...
const SalesVsRefundChart = () => {
  ...
  const isEmpty = !data || data.length === 0 || data.every((entry) => entry.value === 0);
  ...
  return (
    <div className="bg-orange-50 border border-orange-200 rounded-2xl shadow-md p-4 sm:p-6 mb-6 w-full">
      ...
      {isEmpty ? (
        <div className="h-[300px] flex items-center justify-center text-gray-400 italic">No sales data available</div>
      ) : (
        <ResponsiveContainer width="100%" height={300}>
          <PieChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <Tooltip contentStyle={{ backgroundColor: "#fff7ed", borderColor: "#fb923c", fontSize: "14px" }} formatter={(value, name) => [`${value}`, `${name}`]} />
            <Legend verticalAlign="bottom" height={40} formatter={(value, entry) => (
              <span style={{ color: entry.color }}>
                {value}: {((entry.payload.value / data.reduce((a, b) => a + b.value, 0)) * 100).toFixed(0)}%
              </span>
            )} />
