...
const SellerProfile = () => {
  const { authToken, setCurrentSeller, logout } = useAuth();
  ...
  const fetchProfileData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (handleUnauthorizedError(metricsError)) {
          return;
        }
        toast.error(
          metricsError?.response?.data?.message || "Failed to load metrics",
        );
      }
    } catch (error) {
  ...
  }, [authToken, applyProfileToForm, setCurrentSeller, handleUnauthorizedError]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.error(
          metricsError?.response?.data?.message || "Failed to refresh metrics",
        );
      }
...
//   const baseMonthlyTarget = Number(metrics.monthlyTarget || 0);
//   const targetAmount =
//     timeFilter === "yearly"
...
//         ? 0
//         : baseMonthlyTarget;
//   const achievedAmount = Number(metrics.thisMonthSales || 0);
...
//         secondary:
//           pendingFollowupsAmount > 0
//             ? `Amount: ${formatCurrency(pendingFollowupsAmount)}`
//             : undefined,
//         icon: ListChecks,
...
//         secondary:
//           totalConvertedAmount > 0
//             ? `Amount: ${formatCurrency(totalConvertedAmount)}`
//             : undefined,
//         icon: FileText,
...
//             onClick={() => (isEditing ? handleSave() : setIsEditing(true))}
//             disabled={saving}
//             className="rounded-full border border-orange-200 bg-white/80 px-3 py-1.5 text-sm font-semibold text-orange-800 shadow-sm backdrop-blur-sm transition hover:bg-orange-50 disabled:cursor-not-allowed disabled:opacity-60"
//           >
...
//           <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
//             <label className="space-y-1">
//               <span className="text-xs font-semibold uppercase tracking-widest text-orange-900">
//                 Full Name
//               </span>
//               <div className="relative">
